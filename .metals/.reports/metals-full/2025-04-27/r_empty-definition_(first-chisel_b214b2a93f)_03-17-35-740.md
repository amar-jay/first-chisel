error id: file://<WORKSPACE>/mini-gpu/dispatch.scala:64
...
class Dispatcher(
   ...
   private val channelState = RegInit(VecInit.fill(numChannels){ChannelStates.IDLE})
	val currentConsumer = Vec(NUM_CHANNELS, UInt(log2Ceil(NUM_CONSUMERS).W)) // Equivalent to reg [$clog2(NUM_CONSUMERS)-1:0] current_consumer [NUM_CHANNELS-1:0]
	val channelServingConsumer = UInt(NUM_CHANNELS.W) // Equivalent to reg [NUM_CONSUMERS-1:0] channel_serving_consumer
