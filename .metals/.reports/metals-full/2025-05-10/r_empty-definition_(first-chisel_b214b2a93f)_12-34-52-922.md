error id: file://<WORKSPACE>/riscv/decoder.scala:137
...
non-local guesses:
	 ...
	 -chisel3/elsewhen#
	 -chisel3/elsewhen().
	 -chisel3/util/elsewhen.
	 -chisel3/util/elsewhen#
	 -chisel3/util/elsewhen().
	 -chiseltest/elsewhen.
	 -chiseltest/elsewhen#
	 -chiseltest/elsewhen().
	 -riscv/Instructions.elsewhen.
	 -riscv/Instructions.elsewhen#
	 -riscv/Instructions.elsewhen().
	 -elsewhen.
	 -elsewhen#
	 -elsewhen().
	 -scala/Predef.elsewhen.
	 -scala/Predef.elsewhen#
	 -scala/Predef.elsewhen().
offset: 4583
...
class InstructionDecoder extends Module {
  ...
  // Decode the instruction based on the opcode
   when (opcode === Opcodes.R_TYPE) {
      ...
      switch (funct3) {
        when (funct3 === "h0".U) { io.mem_type := I_I_LB }
		}.elsewhen (funct3 === "h1".U) { io.mem_type := I_I_LH }
        }.elsewhen  (funct3 === "h2".U) { io.mem_type := I_I_LW }
        }.elsewhen  (funct3 === "h4".U) { io.mem_type := I_I_LBU }
        }.@@elsewhen  (funct3 === "h5".U) { io.mem_type := I_I_LHU }
      }
		// .otherwise {
		//   io.illegal := true.B
      // }
      ...
      switch (funct3) {
        is ("h0".U) { io.mem_type := S_I_SB }
        is ("h1".U) { io.mem_type := S_I_SH }
        is ("h2".U) { io.mem_type := S_I_SW }
        default { io.illegal := true.B }
      }
      ...
      switch (funct3) {
        is ("h0".U) { io._op := B_I_BEQ }
        is ("h1".U) { io._op := B_I_BNE }
        is ("h4".U) { io._op := B_I_BLT }
        is ("h5".U) { io._op := B_I_BGE }
        is ("h6".U) { io._op := B_I_BLTU }
        is ("h7".U) { io._op := B_I_BGEU }
		  *  { io.illegal := true.B }
      }
