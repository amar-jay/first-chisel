error id: file://<WORKSPACE>/riscv/decoder.scala:86
...
class InstructionDecoder extends Module {
  ...
  // Decode the instruction based on the opcode
   when (opcode === Opcodes.R_TYPE) {
      ...
      switch (funct3) {
        is ("h0".U) { io.mem_type := I_I_LB }
        is ("h1".U) { io.mem_type := I_I_LH }
        is ("h2".U) { io.mem_type := I_I_LW }
        is ("h4".U) { io.mem_type := I_I_LBU }
        is ("h5".U) { io.mem_type := I_I_LHU }
      }
    }
		.otherwise {
		  io.illegal := true.B
