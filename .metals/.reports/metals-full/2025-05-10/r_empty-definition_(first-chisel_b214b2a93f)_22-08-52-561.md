error id: file://<WORKSPACE>/riscv/register.scala:30
...
class ReadPortIo(xlen: Int, mem_add_len:Int) extends Bundle with DecoupledIO {
  val en    = Input(Bool())
  val addr  = Input(UInt(mem_add_len.W))
  val data  = Output(SInt(xlen.W))
}

class WritePortIo(xlen: Int, mem_add_len:Int) extends Bundle with []] {
  val en    = Input(Bool())
  val addr  = Input(UInt(mem_add_len.W))
  val data  = Input(SInt(xlen.W))
}

class RegIo(xlen: Int = Constants.DATA_WIDTH, numRegs: Int = 3@@2) extends Bundle {
