error id: file://<WORKSPACE>/riscv/alu.scala:38
...
class ALU extends Module {
  ...
  switch(io.op) {
    ...
    is(ALUOps.SRA)  { io.out := (a_signed >> shamt).asUInt }
    
    // Comparison operations
    is(ALUOps.SLT)  { io.out := Mux(a_signed < b_signed, 1.U, 0.U) }
    is(ALUOps.SLTU) { io.out := Mux(io.a < io.b, 1.U, 0.U) }
    is(ALUOps.SEQ)  { io.out := Mux(io.a === io.b, 1.U, 0.U) }
    is(ALUOps.SNE)  { io.out := Mux(io.a =/= io.b, 1.U, 0.U) }
