...
found definition using fallback; symbol switch
offset: 5981
...
class InstructionDecoder extends Module {
  ...
  // Decode the instruction based on the opcode
   when (opcode === Opcodes.R_TYPE) {
      ...
      when (funct3 === f"h0".U) { io.mem_type := S_I_SB 
   	}.elsewhen (funct3 === f"h1".U) { io.mem_type := S_I_SH 
		}.elsewhen (funct3 === f"h2".U) { io.mem_type := S_I_SW 
