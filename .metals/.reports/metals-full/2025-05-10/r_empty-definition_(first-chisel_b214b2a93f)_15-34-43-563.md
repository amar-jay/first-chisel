error id: file://<WORKSPACE>/riscv/alu.scala:201
...
class ALUTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "handle division by zero" in {
    test(new ALU()) { dut =>
      ...
      dut.io.op.poke(ALUOperations.DIV)
		dut.clock.step()
		dut.io.result.expect((~0.U(Constants.DATA_WIDTH.W))) // -1 in two's complement
      dut.io.overflow.expect(false.B)
		dut.io.zero.expect(false.B) // Zero flag should be set
  ...
  it should "detect division overflow for edge case" in {
    test(new ALU()) { dut =>
      ...
      dut.io.a.poke(BigInt(2).pow(31))
      dut.io.b.poke((~0.U(Constants.DATA_WIDTH.W))) // -1 in two's compleme@@nt
      dut.io.op.poke(ALUOperations.DIV)
      dut.clock.step()
      dut.io.overflow.expect(true.B)
    }
