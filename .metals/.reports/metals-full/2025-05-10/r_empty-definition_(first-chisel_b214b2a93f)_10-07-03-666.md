error id: file://<WORKSPACE>/riscv/decoder.scala:74
...
class InstructionDecoder extends Module {
	...
	switch (opcode) {
			...
			switch (funct3) {
				is("b000".U) { 
					...
					when (funct7(5)) { // checking the 6th bit of funct7 is enough
						io.alu_op := Operations.ALU_SUB
					}.otherwise {
						io.alu_op := Operations.ALU_ADD
					}
				}
				// 0x4 - xor
				is ("h4".U) { io.alu_op := @@Operations.ALU_XOR }
				is ("h6".U) { io.alu_op := Operations.ALU_OR }
				is ("h7".U) { io.alu_op := Operations.ALU_AND }
				is ("h1".U) { io.alu_op := Operations.ALU_SLL }
				is ("h5".U) {
					// srl or sra
					when (funct7(5)) { // checking the 6th bit of funct7 is enough
				...
				is ("h2".U) { io.alu_op := Operations.ALU_SLT }
				is ("h3".U) { io.alu_op := Operations.ALU_SLTU }
		...
		is (Opcodes.I_TYPE1){ // ALU immediate operations
			...
			switch (funct3) {
				is ("b000".U) { io.alu_op := Operations.I_ADDI }
