			...
			when(inst_fetcher.io._op === Instructions.I_I_JALR) {
               ...
               switch(inst_fetcher.io._op) {
                  is(Instructions.I_I_LB) {
                     // Sign-extended byte
                     regfile.io.write.data := Cat(Fill(24, mem_read_data(7)), mem_read_data(7, 0))
                  }
                  is(Instructions.I_I_LH) {
                     // Sign-extended halfword
                     regfile.io.write.data := Cat(Fill(16, mem_read_data(15)), mem_read_data(15, 0))
                  }
                  ...
                  is(Instructions.I_I_LBU) {
                  	// Zero-extended byte
                     regfile.io.write.data := Cat(0.U(24.W), mem_read_data(7, 0))
                  }
                  is(Instructions.I_I_LHU) {
                     // Zero-extended halfword
                     regfile.io.write.data := Cat(0.U(16.W), mem_read_data(15, 0))
                  }
                }
			}.otherwise{                
